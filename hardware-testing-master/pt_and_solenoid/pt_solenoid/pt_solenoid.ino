...
const long solenoid_interval = 1000;
bool solenoid_state = false;
long temp_wait_time = 0;

void setup() {
  Serial.begin(9600);
  pinMode(solenoid, OUTPUT);
  digitalWrite(solenoid, LOW); // Initally have the solenoid off
...
void loop() {
  ...
  Serial.println(" psi");  
  delay(interval);
  temp_wait_time += interval;
  if (temp_wait_time == solenoid_interval) {
    if (solenoid_state == false) {
